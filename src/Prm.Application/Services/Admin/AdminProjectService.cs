...
public class AdminProjectService : IAdminProjectService
{
    ...
    public async Task<ProjectListItemDto> UpdateAsync(int id, UpdateProjectRequest request, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<MilestoneDto> AddMilestoneAsync(int projectId, AddMilestoneRequest request, CancellationToken cancellationToken = default)
    {
        var project = EntityGuard.EnsureFound(
            await _projects.GetByIdAsync(projectId, cancellationToken),
            ErrorMessages.ProjectNotFound);
