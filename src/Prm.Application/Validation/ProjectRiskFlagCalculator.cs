...
public static class ProjectRiskFlagCalculator
{
    ...
    public static IReadOnlyList<RiskFlag> Calculate(
        ...
        IReadOnlyDictionary<int, int> employeeTotalUtilisation,
        ...
        DateOnly today)
    {
        ...
        flags.Add(GetAllocationFlag(projectActiveAllocations, employeeTotalUtilisation));
        ...
        int maxWeeklyHours)
    {
        ...
        foreach (var allocation in activeAllocations)
        {
            ...
            if (loggedHours < expectedHours * 0.5m)
            {
                yield return new RiskFlag(
                    "LOW_HOURS",
                    $"{allocation.Employee.User.FullName} logged only {loggedHours} hrs last week (expected {expectedHours} hrs).",
                    true);
            }
        }
    }
        ...
        IReadOnlyDictionary<int, int> employeeTotalUtilisation)
    {
        var overAllocated = projectActiveAllocations
            ...
            .Distinct()
            .Any(employeeId =>
                employeeTotalUtilisation.TryGetValue(employeeId, out var total)
